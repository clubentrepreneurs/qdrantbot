...
# --- STYLE CSS (Masquage total du haut et du bas) ---
hide_style = """
    <style>
    /* Masque le bandeau noir/gris du haut (GitHub, Fork, etc.) */
    header {visibility: hidden !important;}
    [data-testid="stHeader"] {display: none !important;}
    
    /* Masque la décoration arc-en-ciel */
    [data-testid="stDecoration"] {display: none !important;}
    
    /* Masque le menu et le footer */
    #MainMenu {visibility: hidden;}
    footer {visibility: hidden !important;}
    
    /* Supprime les espaces inutiles en haut */
    .block-container {
        padding-top: 1rem;
        padding-bottom: 0rem;
    }
    
    /* Masque le bouton Deploy si présent */
    .stDeployButton {display:none;}
    </style>
    """
st.markdown(hide_style, unsafe_allow_html=True)

# --- 2. TITRE PERSONNALISÉ (Dans le corps de la page) ---
    ...
    with st.chat_message("assistant"):
        if not texte_universite:
            ...
            try:
                ...
                st.error(f"Erreur : {e}")client = Mistral(api_key=st.secrets["MISTRAL_API_KEY"])
MODEL = "mistral-small-latest"
